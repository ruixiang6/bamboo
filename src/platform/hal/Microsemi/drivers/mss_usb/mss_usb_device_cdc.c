...
mss_usbd_cdc_app_cb_t* usbd_cdc_app_ops;
...
#else
#define VCP_CONFIG_DESCR_LENGTH		32
uint8_t cdc_conf_descr[] = {
    ...
    0xDFu,                                              /* bmAttributes */
    0x00u,                                              /* bMaxPower */    
    /*-------------- Interface 0 - Data class interface descriptor -----------*/
    USB_STD_INTERFACE_DESCR_LEN,                        /* bLength: Endpoint Descriptor size */
    USB_INTERFACE_DESCRIPTOR_TYPE,                      /* bDescriptorType: */
    0x00,                                               /* bInterfaceNumber: Number of this Interface */
    0x00,                                               /* bAlternateSetting: Alternate setting */
...
MSS_USBD_CDC_init
(
    mss_usbd_cdc_app_cb_t* cdc_user_cb,
    mss_usb_device_speed_t speed
)
{
    usbd_cdc_app_ops = cdc_user_cb;
...
{
	//MSS_USBD_tx_ep_flush_fifo(CDC_BULK_IN_EP);
    MSS_USBD_tx_ep_write(CDC_BULK_IN_EP, buf, length);
...
{
    MSS_USBD_rx_ep_read_prepare(CDC_BULK_OUT_EP, buf, length);
...
)
{
     ...
     */
#if 0
    MSS_USBD_tx_ep_configure(CDC_INTR_IN_EP,
                             CDC_INTR_IN_EP_FIFO_ADDR,
                             CDC_INTR_IN_EP_FIFO_SIZE,
                             CDC_INTR_IN_EP_MAX_PKT_SIZE,
                             1,
                             DMA_DISABLE,
                             ...
                             ADD_ZLP_TO_XFR);
#endif
    MSS_USBD_tx_ep_configure(CDC_BULK_IN_EP,
                             CDC_BULK_IN_EP_FIFO_ADDR,
                             CDC_BULK_IN_EP_FIFO_SIZE,
                             CDC_BULK_IN_EP_MAX_PKT_SIZE,
                             1,
                             DMA_ENABLE,
    ...
    MSS_USBD_rx_ep_configure(CDC_BULK_OUT_EP,
                             CDC_BULK_OUT_EP_FIFO_ADDR,
                             CDC_BULK_OUT_EP_FIFO_SIZE,
                             CDC_BULK_OUT_EP_MAX_PKT_SIZE,
                             1,
                             DMA_ENABLE,
    ...
    usbd_cdc_app_ops->usb_cdc_init();

    usbd_cdc_app_ops->usb_cdc_notification(&buf_p, &len);

    if(buf_p)
    {
        MSS_USBD_tx_ep_write(CDC_INTR_IN_EP, buf_p, len);
    }
    return 1;
}
...
{
    if(USB_SUCCESS == usbd_cdc_app_ops->usb_cdc_process_request(setup_pkt,
                                                                buf_pp,
                                                                length))
    {
        return USB_SUCCESS;
...
)
{
    ...
    else if(num == CDC_BULK_IN_EP)
    {
        usbd_cdc_app_ops->usb_cdc_tx_complete(status);
    }
    else
    {
        return USB_FAIL;
...
{
    if(num == CDC_BULK_OUT_EP)
    {
        usbd_cdc_app_ops->usb_cdc_rx(status, rx_count);
    }
    else
    {
        return USB_FAIL;
...
{
    if(0 != usbd_cdc_app_ops->usb_cdc_cep_tx_complete)
        usbd_cdc_app_ops->usb_cdc_cep_tx_complete(status);
    return USB_SUCCESS;
}
...
{
    if(0 != usbd_cdc_app_ops->usb_cdc_cep_rx)
        usbd_cdc_app_ops->usb_cdc_cep_rx(status);
    return USB_SUCCESS;
}
