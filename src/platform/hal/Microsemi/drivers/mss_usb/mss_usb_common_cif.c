...
)
{
    ...
    while(irq_num)
    {
        ...
        if((irq_num) && (irq_num & MSS_USB_WORD_BIT_0_MASK))
        {
			...
			//hal_uart_send_char(UART_DEBUG, 'D');
			//if (!MSS_USB_CIF_is_txepfifo_notempty(ep_num))
			//{
			//  	hal_uart_send_char(UART_DEBUG, 'H');
        ...
        if(dma_irq & 0x01u)
        {
            ...
            if(dma_dir == MSS_USB_DMA_READ)    /*TX EP*/
            {
                ...
                */
				...
				//再赋值一次，从DMA搬移到TX_FIFO中
				gd_tx_ep[ep_num]->txn_length = (increamented_addr -
                                               ((uint32_t)(gd_tx_ep[ep_num]->buf_addr)));
				if (gd_tx_ep[ep_num]->txn_length != gd_tx_ep[ep_num]->xfr_length)
				{
					DBG_ASSERT(0);				  	
                }				
                /*
    ...
    if((core_ep->num) && (core_ep->buf_addr != 0))
    {
               ...
               (core_ep->txn_length <= core_ep->fifo_size))
        {
            ...
            */
            ...
            {
			  	MSS_USB_CIF_tx_ep_disable_irq(core_ep->num);
