...
)
{
    ...
    if(err_check == USB_SUCCESS)
    {
		...
		{
			device_tx_ep[ep_num].fifo_size = fifo_size/2;
		}
		else
		{
...
)
{
    ...
    if(err_check == USB_SUCCESS)
    {
		...
		if (device_rx_ep[ep_num].dpb_enable == DPB_ENABLE)
        {
			device_rx_ep[ep_num].fifo_size = fifo_size/2;
        }
        else
        {
...
)
{
    ...
    if((ep_num) && (addr != 0))
    {
		...
		device_tx_ep[ep_num].txn_length = 0u;
        ...
        if(device_tx_ep[ep_num].xfr_type == MSS_USB_XFR_BULK)
        {
            ...
            */
			...
			if (device_tx_ep[ep_num].dma_enable == DMA_ENABLE)
			{
			  	device_tx_ep[ep_num].txn_length = 0;
			}
        }
...
)
{
    ...
    else
    {
        ...
        if(DMA_ENABLE == device_tx_ep[num].dma_enable)
        {
            ...
            //transfer_complete = 1u;
		  	device_tx_ep[num].txn_count = device_tx_ep[num].txn_length;
            device_tx_ep[num].xfr_count += device_tx_ep[num].txn_length;
            ...
            if(device_tx_ep[num].xfr_type == MSS_USB_XFR_BULK)
            {
                ...
                else if(device_tx_ep[num].xfr_count == device_tx_ep[num].xfr_length)
                {
                    if(device_tx_ep[num].add_zlp == ADD_ZLP_TO_XFR)
                    {
						...
						else if((device_tx_ep[num].txn_count%device_tx_ep[num].max_pkt_size) == 0)
						{
						  	//·¢ËÍ0×Ö½Ú°ü
							transfer_complete = 0u;														
						}
    ...
    if(transfer_complete == 1)
    {
        #ifndef MSS_USB_DEVICE_TEST_MODE
	  		device_tx_ep[num].txn_count = 0;
			device_tx_ep[num].txn_length = 0;
			device_tx_ep[num].buf_addr = NULL;
			//MSS_USB_CIF_tx_ep_clr_data_tog(num);
