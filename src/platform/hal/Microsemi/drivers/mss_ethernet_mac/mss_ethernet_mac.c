 ...
 *
 * SVN $Revision: 5686 $
 * SVN $Date: 2013-04-12 13:24:08 +0100 (Fri, 12 Apr 2013) $
 */
#include <stdio.h>
...
static void update_mac_cfg(void)
{
    ...
    link_up = eth_phy_get_link_status(&speed, &fullduplex);
    if(link_up != 0u)
    {
...
)
{
    ...
    link_up = eth_phy_get_link_status(&link_speed, &link_fullduplex);
    if(link_up != 0u)
    {
